...
<div class="pt-10">
  <div class="max-w-md shadow-md mx-auto p-4 bg-white rounded-lg ">
    <div class="flex justify-between pb-4 items-center">
      <h1 class="text-2xl grow-[2] text-center font-bold">Expense Tracker</h1>
      <Faq/>
    </div>
    ...
    <div>
      <button  on:click={()=>{showDropDownMenu()}} type="button" class="inline-flex w-full justify-center h-10  rounded-md rounded-tr-none rounded-br-none bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50" id="menu-button" aria-expanded="true" aria-haspopup="true">
       {typeOfTransaction}
   ...
   {#if showDropDown}
    <div class="absolute right-0 z-10 mt-2 w-22 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
      <div class="py-1" role="none">
        <button on:click={()=>{onTransactionTypeChange(true)}} class="mt-2 mb-2 block px-7 py-2 text-sm text-gray-700 hover:bg-gray-400" role="menuitem" tabindex="-1" id="menu-item-0">Credit</button>
        <button on:click={()=>{onTransactionTypeChange(false)}} class="mt-2 mb-2 block px-7 py-2 text-sm text-gray-700 hover:bg-gray-400" role="menuitem" tabindex="-1" id="menu-item-1">Debit</button>
      </div>
    </div>
    {/if}
  </div>
      </div>
      <input on:keypress={(e) => handleEnter(e)}
